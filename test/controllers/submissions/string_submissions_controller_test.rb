...
class StringSubmissionsControllerTest < ActionDispatch::IntegrationTest
	setup { @string_submission = string_submissions(:one) }
	...
	test 'should create string_submission' do
		assert_difference('StringSubmission.count') do
			post string_submissions_url,
			     params: {
					string_submission: {
						answer: @string_submission.answer,
						string_question_id:
							@string_submission.string_question_id
					}
			     },
			     as: :json
		end
	...
	test 'should update string_submission' do
		patch string_submission_url(@string_submission),
		      params: {
				string_submission: {
					answer: @string_submission.answer,
					string_question_id: @string_submission.string_question_id
				}
		      },
		      as: :json
		assert_response 200
